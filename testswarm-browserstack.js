...
var TestSwarmBrowserStackInteg = {
	// We need to map the useragent IDs that TestSwarm uses
	// to browser definitions in BrowserStack.
	browserMap: {
		1: {
			name: 'chrome',
			version: '15.0'
	...
	updateBrowsers: function (currentWorkers, neededWorkers) {
		...
		for (i in self.browserMap) {
			isStarted = self.isWorkerStarted(self.browserMap[i], currentWorkers);
			isNeeded = neededWorkers.indexOf(parseInt(i, 10)) > -1 ? true : false;
